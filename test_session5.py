...
def test_session5_time_it_print():
    assert print(1, 2, 3, sep='-', end=' ***\n') == time_it(custom_print, 1, 2, 3,
                                                            sep='-', end=' ***\n', repetitions=5)[0], "time_it can't time print function"
...
def test_session5_time_it_squared_power_list():
    ...
    my_list = list(range(r1, r2))

    assert [pow_no**x for x in my_list] == time_it(squared_power_list, pow_no, start=r1, end=r2)[
        0], "time_it can't time squared_power_list function"


def test_session5_time_it_polygon_area():
    length = random.randint(1, 1000)
    sides = random.randint(3, 6)
    assert round((sides * (length ** 2) / (4 * math.tan(math.pi / sides))), 2) == round(time_it(polygon_area, length,
                                                                                                sides=sides)[0], 2), "time_it can't time polygon_area function"


def test_session5_time_it_temp_converter():
    temp = random.uniform(-459.66, 1000)
    assert math.isclose(time_it(temp_converter, temp)[
                        0], ((temp - 32) / 1.8)) == True, "time_it can't time temp_converter function"
...
def test_session5_time_it_speed_converter():
    ...
    assert time_it(speed_converter, speed, dist='KM', time='HR')[
        0] == round(speed), "time_it can't time speed_converter function "
...
def test_session5_squared_power_list_output():
    ...
    assert session5.squared_power_list(1, start=1, end=5) == [
        1, 1, 1, 1], "squared_power_list is not working as expected"
...
def test_session5_polygon_area_sides_values():
    ...
    length = random.randint(1, 1000)
    sides = 3
    assert math.isclose(time_it(polygon_area, length, sides=sides)[
                        0], (math.sqrt(3)/4)*math.pow(length, 2)) == True, 'area calculated in incorrect'
...
def test_session5_temp_converter():
    ...
    #     session5.temp_converter(1, 2, temp_given_in='F')
...
def test_session5_speed_converter_output_yrd_to():
    ...
    assert session5.speed_converter(
        6000, dist='YRD', time='MIN') == 109361, "speed_converter is not working as expected"
    assert session5.speed_converter(
        100, dist='YRD', time='HR') == 109361, "speed_converter is not working as expected"
    assert session5.speed_converter(
        100, dist='YRD', time='DAY') == 2624662, "speed_converter is not working as expected"
